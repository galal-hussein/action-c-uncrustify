...
while read -r FILENAME; do
    ...
    set +e
    OUT=$(uncrustify --check${CONFIG} -f ${FILENAME} 2>&1 | grep -e ^FAIL -e ^PASS | awk '{ print $2 }'; exit ${PIPESTATUS[0]})
    RETURN_VAL=$?
...
done < <(git diff --name-status --diff-filter=AM origin/${DEFAULT_BRANCH}...${BRANCH_NAME} -- '*.c' '*.h' | awk '{ print $2 }' )
